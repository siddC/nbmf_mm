...
orientation : {"dir-beta", "beta-dir"}, default="dir-beta"
    ...
    prior. See the orientation descriptions above. Several case‑insensitive
    aliases are accepted, e.g. "Dir‑Beta", "Aspect Bernoulli" → "dir‑beta";
...
projection_backend : {"auto", "numpy", ...}, default="auto"
    Backend selector for the projection.
...
def _compute_A_B(Y: np.ndarray, P: np.ndarray, mask: Optional[np.ndarray], eps: float) -> Tuple[np.ndarray, np.ndarray]:
    ...
    A = np.zeros_like(Y); B = np.zeros_like(Y); nz = mask.astype(bool)
    A[nz] = Y[nz] / P[nz]
...
def _project_rows_to_simplex(W: np.ndarray) -> np.ndarray:
    ...
    for m in range(M):
        ...
        if rho_idx.size == 0:
            Wp[m].fill(1.0 / K); continue
        rho = rho_idx[-1]; theta = (cssv[rho] - 1.0) / (rho + 1)
        x = y - theta; x[x < 0.0] = 0.0
        s = x.sum(); Wp[m] = (x / s) if s != 0.0 else np.full_like(x, 1.0 / K)
    return _clip01(Wp)
...
def _project_cols_to_simplex(H: np.ndarray) -> np.ndarray:
    ...
    for n in range(N):
        ...
        rho_idx = np.nonzero(u + (1.0 - cssv) / (np.arange(K) + 1) > 0.0)[0]
        if rho_idx.size == 0:
            Hp[:, n].fill(1.0 / K); continue
        rho = rho_idx[-1]; theta = (cssv[rho] - 1.0) / (rho + 1)
        x = y - theta; x[x < 0.0] = 0.0
        s = x.sum(); Hp[:, n] = (x / s) if s != 0.0 else np.full(K, 1.0 / K)
    return _clip01(Hp)


def _canon_orientation(o: str) -> str:
    s = str(o).strip().lower().replace("_", "-").replace(" ", "-")
    dirbeta = {"dir-beta", "dirbeta", "dir-beta", "aspect-bernoulli", "aspect", "aspectbernoulli"}
    betadir = {"beta-dir", "betadir", "beta-dir", "binary-ica", "binaryica", "bica"}
    if s in dirbeta: return "dir-beta"
    if s in betadir: return "beta-dir"
    if s in {"dir-beta", "beta-dir"}: return s
...
class NBMF:
    ...
    n_init: int = 1
    projection_method: str = "normalize"   # default to MM‑faithful path
    projection_backend: str = "auto"
    use_numexpr: bool = False
    use_numba: bool = False                # accepted, unused (API compatibility)
    ...
    def fit(self, X: np.ndarray, mask: Optional[np.ndarray] = None):
        ...
        if mask is not None:
            ...
            if mask.shape != X.shape: raise ValueError("mask must have the same shape as X.")

        M, N = X.shape; K = int(self.n_components)
        best_obj = np.inf; best_W = None; best_H = None; best_hist: list[float] = []
        ...
        for init_idx in range(max(1, int(self.n_init))):
            ...
            for it in range(1, self.max_iter + 1):
                ...
                if self.orientation == "beta-dir":
                    ...
                    H = _clip01(C / (C + D + self.eps), self.eps)

                    # ---- recompute with updated H  (critical for MM)
                    P = _clip01(W @ H, self.eps)
                    A, B = _compute_A_B(X, P, mask, self.eps)
                    ...
                    if self.projection_method == "duchi":
                        ...
                        rs = W_new.sum(axis=1, keepdims=True); rs[rs == 0.0] = 1.0
                    ...
                    # ---- recompute with updated W  (critical for MM)
                    P = _clip01(W @ H, self.eps)
                    A, B = _compute_A_B(X, P, mask, self.eps)
                    ...
                    if self.projection_method == "duchi":
                        ...
                        cs = H_new.sum(axis=0, keepdims=True); cs[cs == 0.0] = 1.0
                        H = _clip01(H_new / cs, self.eps)

                # objective trace
                P = _clip01(W @ H, self.eps)
                nll = _bernoulli_nll(X, P, mask, average=False, eps=self.eps)
                obj = nll - (_beta_log_prior(H, self.alpha, self.beta, self.eps) if self.orientation == "beta-dir"
            ...
            if obj < best_obj:
                best_obj = obj; best_W = W; best_H = H; best_hist = hist; self.n_iter_ = it

        self.W_ = best_W; self.components_ = best_H; self.objective_history_ = best_hist
    ...
    def transform(self, X: np.ndarray, mask: Optional[np.ndarray] = None, max_iter: int = 500, tol: float = 1e-6) -> np.ndarray:
        if self.components_ is None: raise RuntimeError("Call fit() before transform().")
        H = self.components_; X = np.asarray(X, dtype=float)
        M, N = X.shape; K = H.shape[0]
        rng = np.random.default_rng(self.random_state)
    ...
    def inverse_transform(self, W: np.ndarray) -> np.ndarray:
        if self.components_ is None: raise RuntimeError("Call fit() before inverse_transform().")
    ...
    def _random_init(self, rng: np.random.Generator, M: int, N: int, K: int) -> Tuple[np.ndarray, np.ndarray]:
        if self.orientation == "beta-dir":
            ...
            H = _project_cols_to_simplex(rng.random((K, N)))
            W = _clip01(rng.random((M, K)), self.eps)
        else:
            raise ValueError('orientation must be "beta-dir" or "dir-beta"')
        return W, H
